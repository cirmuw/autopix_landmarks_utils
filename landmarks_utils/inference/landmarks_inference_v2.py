...
def main():
    ...
    lm_coordinate_names_generic = [f"landmark__{i}_{axis}" for i in range(len(all_pred_landmarks_transformed[0])) for axis in ["x", "y"]]
    ...
    landmarks_df_separate["img"] = landmarks_df_separate["image_path"].apply(lambda x: Path(x).stem)
    #landmarks_df_separate["img"] = landmarks_df_separate["file_name"] 
